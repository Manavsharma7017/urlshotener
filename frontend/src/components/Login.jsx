...
const Login=()=>{
  ...
  const handlelogic= async()=>{
   ...
   axios.post("http://localhost:3000/login",
     ...
     else if(response.data.msg=="email or passward is incorrect"){
             alert("email or passward is incorrect")
             setloder(0)
     }
     else{
  ...
  <CardFooter>
   <Button onClick={handlelogic}>{loder ?<BeatLoader size={10} color="#36d7b7"></BeatLoader>:"login"}</Button>
  </CardFooter>
</Card>
</>
  )
